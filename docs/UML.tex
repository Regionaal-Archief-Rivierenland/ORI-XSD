...
\begin{tikzpicture}[
  ...
  \node (stemmingOverPersonenGegevens) [ORI object, right = 1.2cm of besluit, align=center, draw=gray]
   ...
   \node (informatieobject) [ORI object, above right = -4.6cm and 5cm of agendapunt, align=center]
        {
          \objectheader{Relatieklasse}{Informatieobject}

          \attribute{Informatieobject type}{\keuze{enumeratie Informatieobject type}}{[0..1]}
        };

  %% inner MDTO rectangle
  \node (mdto-inner) [rectangle, draw, ultra thick, right = 3cm of informatieobject, inner sep=40pt, fill=githubgrey, align=center]
        {
          {\huge MDTO}
        };
  ...
  \node (mdto) [fit=(mdto-inner), draw, ultra thick, inner sep=1.1cm] {};
        ...
        coordinate[pos=0.26] (tempcoordinate)
        node [pos=0.26, above] {heeft als bijlage}
        node [pos=0.26, below] {\umltype{Relatiesoort}};
        % Draw the dashed arrow
        ...
        node [anchor=center, above, pos=0.325] {heeft als bijlage}
        node [anchor=center, below, pos=0.325] {\umltype{Relatiesoort}}
        coordinate[pos=0.325] (tempcoordinate) %% define tmp halfway coord
        ($(mdto.north)+(-2.8cm,0)$);
