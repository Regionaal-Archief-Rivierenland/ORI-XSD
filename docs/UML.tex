...
\begin{tikzpicture}[
  ...
  \node (stemmingOverPersonenGegevens) [ORI object, right = 1.2cm of besluit, align=center, draw=gray]
   ...
   \node (informatieobject) [ORI object, above right = 6cm and -0.5cm of stemmingOverPersonenGegevens, align=center]
        {
          \objectheader{Objecttype}{Informatieobject}
  ...
  \node (mdto) [rectangle, draw, ultra thick, right = 3cm of informatieobject, inner sep=50pt, double, double distance = 1cm, fill=githubgrey, align=center]
        ...
        ($(informatieobject.south)+(0,-0.5cm)$);
...
\draw[relatiepijl] 
    (agendapunt.east) -- ++(2cm,0) -- (informatieobject.west)
    node [pos=0.08, above, fill=none] {heeft als bijlage}
    node [pos=0.08, below] {\umltype{Relatiesoort}};
        ...
        ($(informatieobject.north)+(0cm,0.5cm)$);
        ...
        ($(informatieobject.north)+(-1.2cm,0.5cm)$);
        %% dashed line
        ...
        %% vergadering -> informatieobject
        \draw[relatiepijl] ($(vergadering.east)+(0, 3)$) -|
        ($(vergadering.east)+(11.0,6)$) --
        node [anchor=center, above, pos=0.12] {heeft als bijlage}
        node [anchor=center, below, pos=0.12] {\umltype{Relatiesoort}}
        ($(informatieobject.north)-(3.3cm, 0cm)$);
       ...
       %% deelnemer -> vergadering
        ...
        (natuurlijkpersoon);
        %% deelnemer -> stemming
        \draw[dashed, very thick] ($(deelnemer.east)!0.5!(stemming.west) + (0, .50)$) -- (stem);
