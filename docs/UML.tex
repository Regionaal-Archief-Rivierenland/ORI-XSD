...
\begin{tikzpicture}[
  ...
  \node (agendapunt) [ORI object, align=center]
   ...
   \node (stem) [ORI object, align=center, at = ($(deelnemer.east)!0.5!(stemming.west)$), yshift=4cm]
        {
          \objectheader{Relatieklasse}{Stem}

          \attribute{ID}{string}{[0..1]} \\
          \attribute{\textbf{Keuze stemming}}{\keuze{Voor} | \keuze{Tegen} | ...}{}
        };
   \node (spreekfragment) [ORI object, above = 17.5cm of stem]
        {
   ...
   \node (naam) [ORI object, below = 2.2cm of natuurlijkpersoon, align=center, draw=gray]
        {
   ...
   \node (fractielidmaatschap) [ORI object, below = 2.2cm of deelnemer, align=center, draw=gray]
        {
          \objectheader{Gegevensgroeptype}{Fractielidmaatschap}
  ...
  \node (stemmingOverPersonenGegevens) [ORI object, right = 1.2cm of besluit, align=center, draw=gray]
        ...
        \node (informatieobject) [ORI object, align=center, above left = 6.6cm and -15.0cm of besluit]
        {
  ...
  \node (mdto) [rectangle, draw, ultra thick, above right = 6cm and -0.5cm of stemmingOverPersonenGegevens, inner sep=50pt, double, double distance = 1cm, fill=githubgrey, align=center]
        ...
        %% agendapunt -> MDTO
        \draw[relatiepijl] ($(agendapunt.east)+(0,0.0)$) --
        ...
        %% mediabron -> is vastgelegd in
        \draw[relatiepijl] (mediabron) ($(mediabron.east)+(0,0)$)  -|
        ...
        %% dashed line
        \draw[dashed, very thick] (linestart) |- ($(informatieobject.north west)+(0,-0.6)$);

        %% mediabron -> heeft ondertitel bestand
        \draw[relatiepijl] (mediabron) ($(mediabron.east)+(0,1.3)$)  -|
        node [pos=0.18, above] {heeft ondertitelbestand}
        node [pos=0.18, below] {\umltype{Relatiesoort}}
        coordinate[pos=0.26] (linestart4) % coordinate along the line
        ($(mdto.north)+(1.2cm,0.5cm)$);
        %% dashed line
        \draw[dashed, very thick] (linestart4) |- (informatieobject.north west);

        %% fractie -> stemming
        \draw[relatiepijl] (fractie) --
        node [anchor=center, fill=white] {neemt deel aan\\\umltype{Relatiesoort}} (stemming);
        ...
        ($(vergadering.east)+(11.0,6)$) --
        node [anchor=center, fill=white, above, pos=0.12] {heeft als bijlage}
        node [anchor=center, fill=white, below, pos=0.12] {\umltype{Relatiesoort}}
        ($(mdto.north)-(3.3cm, 0cm)$);
       ...
       %% deelnemer -> vergadering
        ...
        (natuurlijkpersoon);
        %% deelnemer -> stemming
        \draw[dashed, very thick] ($(deelnemer.east)!0.5!(stemming.west) + (0, .10)$) -- (stem);
        ...
        \draw[gegevenspijl] (natuurlijkpersoon.south) -- (naam);
        ...
        \draw[relatiepijl] (natuurlijkpersoon) ($(natuurlijkpersoon.south)+(3.9,0)$)  |-
        coordinate[pos=1.0] (linestartfractielidmaatschap) % coordinate along the line
        ($(fractielidmaatschap.south)-(0.0cm,1.9cm)$) -| (fractie.south);
        %% dashed line
