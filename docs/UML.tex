...
\begin{tikzpicture}[
  ...
  \node (mdto) [rectangle, draw, ultra thick, right = 3cm of informatieobject, inner sep=50pt, double, double distance = 1cm, fill=githubgrey, align=center]
        ...
        %% FIXME: place below besluit. That makes your arrows cleaner.
        %% Also, now it looks like these two things have something to do with each other, which they don't 
        ...
        %% agendapunt -> mdto
        \draw[relatiepijl] 
        ($(agendapunt.east) - (0, 2cm)$) -- ++(2cm,0) -| ($(mdto.south) - (2cm, 0cm)$)
        node [pos=0.1, above] {heeft als bijlage}
        node [pos=0.1, below] {\umltype{Relatiesoort}};
